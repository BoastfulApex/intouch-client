...
const Order = () => {
    ...
    const product = useSelector(state =>
        state.products?.value?.find(item => item.Slug === slug)
    );
...
Товар: ${product?.Title || "Неизвестный товар"}
...
Комментарии: ${comment}`
    ...
    return (
        <div className={`popup ${popup.value === "order" ? "active" : ""}`} id="order">
            ...
            <form className="popup-inner order" onSubmit={handleSubmit}>
                ...
                <div className="contact__form-check">
                    ...
                    <div>
                        <label htmlFor="chk5">Подтверждаю свое согласие с условиями </label><Link href="/policy" onClick={() => dispatch(setPopup(""))}>обработки персональных данных </Link>
                    </div>
                </div>
                {error !== "" && <p className='error'>{error}</p>}
                <button className="contact__form-btn btn pop" disabled={loading.value}>Отправить</button>
            </form>
        </div>
    )
}
